...
// Definir constantes para evitar "magic numbers"
...
// Función para verificar si un archivo debe ser cifrado según su extensión
...
void aes_encrypt_gcm(const char *in_filename, const char *out_filename, unsigned char *key) {
    ...
    // Crear contexto de cifrado y inicializarlo
    ...
    // Obtener y escribir tag en el archivo cifrado
...
// Función para cifrar un archivo con AES (modo GCM)
...
int rsa_encrypt_key(RSA *rsa, unsigned char *key, unsigned char *encrypted_key) {
    int result = RSA_public_encrypt(KEY_SIZE, key, encrypted_key,
